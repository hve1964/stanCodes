/* Vectorised Stan model code for varying effects logistic regression */
data {
  /* Dimensions */
...
model {
  ...
  target += bernoulli_logit_lpmf( y | rows_dot_product( beta[gp] , X ) );
}
...
generated quantities {
  ...
  for ( i in 1:N ) {
    yrep[i] = bernoulli_logit_rng( rows_dot_product( beta[gp] , X )[i] );
    
    log_lik[i] = bernoulli_logit_lpmf( y[i] |
      rows_dot_product( beta[gp] , X )[i] );    
  }
}
